...
export class FileSelector {
   ...
   */
  public async showFileSelectorUI() {
   ...
   */
  private async getAllWorkspaceFiles(dir: string): Promise<string[]> {
    const files: string[] = [];
   ...
   */
  private async processSelectedFiles(selectedFilePaths: string[]) {
    try {
      ...
      for (const relativePath of selectedFilePaths) {
        ...
        } catch (fileError: any) { // Type assertion to any for backward compatibility
   ...
   */
  private getWebviewContent(files: string[], webview: vscode.Webview): string {
                ...
                <div class="directory-group">
                  ...
                  <div class="file-list" data-directory="${directory}">
                    ${dirFiles.map((file, index) => {
                      ...
                      let fileIcon = '📄';
                      if (['.js', '.jsx'].includes(fileExt)) fileIcon = '🟨';
                      if (['.ts', '.tsx'].includes(fileExt)) fileIcon = '🟦';
                      if (['.html'].includes(fileExt)) fileIcon = '🌐';
                      if (['.css', '.scss'].includes(fileExt)) fileIcon = '🎨';
                      if (['.json'].includes(fileExt)) fileIcon = '📊';
                      if (['.py'].includes(fileExt)) fileIcon = '🐍';
                      if (['.java'].includes(fileExt)) fileIcon = '☕';
                      if (['.c', '.cpp', '.h'].includes(fileExt)) fileIcon = '⚙️';
                      if (['.md', '.markdown'].includes(fileExt)) fileIcon = '📝';
                      if (['.txt'].includes(fileExt)) fileIcon = '📄';
                      if (['.xml', '.svg'].includes(fileExt)) fileIcon = '🔖';
                      if (['.jpg', '.jpeg', '.png', '.gif', '.bmp'].includes(fileExt)) fileIcon = '🖼️';
                      if (['.pdf'].includes(fileExt)) fileIcon = '📑';
                      if (['.zip', '.rar', '.7z', '.tar', '.gz'].includes(fileExt)) fileIcon = '🗜️';
                      if (['.mp3', '.wav', '.ogg', '.flac'].includes(fileExt)) fileIcon = '🎵';
                      if (['.mp4', '.avi', '.mov', '.wmv'].includes(fileExt)) fileIcon = '🎬';
                      if (['.doc', '.docx'].includes(fileExt)) fileIcon = '📘';
                      if (['.xls', '.xlsx'].includes(fileExt)) fileIcon = '📊';
                      if (['.ppt', '.pptx'].includes(fileExt)) fileIcon = '📊';
                      if (['.exe', '.dll', '.so', '.bin'].includes(fileExt)) fileIcon = '⚙️';
                      if (['.sh', '.bash', '.zsh', '.fish'].includes(fileExt)) fileIcon = '📜';
                      if (['.sql'].includes(fileExt)) fileIcon = '💾';
                      if (['.yml', '.yaml'].includes(fileExt)) fileIcon = '⚙️';
                      if (['.csv'].includes(fileExt)) fileIcon = '📊';
